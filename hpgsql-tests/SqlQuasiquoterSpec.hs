...
spec = do
  describe "Query concatenation" $ do
    it "Concatenating mkQuery queries"
      mkQueryConcatenation
    it "Concatenating #{}-interpolated queries"
      interpolationConcatenation
    it "Concatenating ^{}-embedded queries"
      embeddedQueryConcatenation
    it "Concatenating mixed mkQuery, #{}, and ^{} queries"
      mixedConcatenation
...
checkQueryConcatenation gen = hedgehog $ do
  queries <- forAll $ Gen.list (Range.linear 1 10) gen
  let concatenated = foldr1 (<>) queries
      Query singleQueries = concatenated

  -- Total number of SingleQueries equals sum of input counts
  let inputCount = sum $ map (\(Query qs) -> length qs) queries
  length singleQueries === inputCount

  -- Each SingleQuery must be independently valid
  forM_ (NE.toList singleQueries) $ \(SingleQuery qStr qParams) -> do
