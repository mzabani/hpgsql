...
Some benchmarks show materializing large query results with hpgsql takes 31-33% the time postgresql-simple takes, and 56-65% the time hasql takes (on my computer, Linux x64, GHC 9.10.3, compiled with -O1).
...
hpgsql's binary COPY runs in ~48% the time of postgresql-simple's textual COPY.
