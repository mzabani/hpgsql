...
testFold TestEnv {..} = do
  ...
  forEach
    conn
    "SELECT * FROM generate_series(1,?) a, generate_series(1,?) b"
    (100 :: Int, 50 :: Int)
    $ \(a :: Int, b :: Int) -> do
      xs <- readIORef ref
    ...
    forEach_ conn "SELECT generate_series(1,10)" $ \(Only a) ->
      if a == 5
        then do
          ...
          True <-
            expectError isSyntaxError $
              execute_ conn "asdf"
          True <-
            expectError ST.isFailedTransactionError $
...
testSavepoint TestEnv {..} = do
  True <-
    expectError ST.isNoActiveTransactionError $
      withSavepoint conn $
        return ()
